...
def main():
    ...
    parser.add_argument('--max_epoch',type=int, default=100) # Changed from 80 to 100 cuz we aren't using warmup training exactly.
    ...
    if not os.path.exists('IDDP_model/{}'.format(datetime)):
        os.makedirs('IDDP_model/{}'.format(datetime))
    if args.cpu or not torch.cuda.is_available():
        device=torch.device("cpu")
    # creation of the dataloaders for train and validation
    mean_tr, std_tr, mean_speed, std_speed = baselineUtils.generate_mean_std()
    train_dataset=baselineUtils.create_iddp_dataset(mean_tr,std_tr,mean_speed,std_speed,dataset='iddp',flag='train',**args.model_opts)

    val_dataset=baselineUtils.create_iddp_dataset(mean_tr,std_tr,mean_speed,std_speed,dataset='iddp',flag='val',**args.model_opts)
    ...
    #################################################################################
    # Since we don't have a warmup model right now, ignoring this code.
    ...
    ###################################################################################
    # Adding this warmup scheduler to compensate for the lack of the warmup phase
    class WarmupScheduler(_LRScheduler):
        def __init__(self, optimizer, warmup_epochs, initial_lr, final_lr, last_epoch=-1):
        ...
        def get_lr(self):
            if self.last_epoch < self.warmup_epochs:
                lr = self.initial_lr + (self.final_lr - self.initial_lr) * (self.last_epoch / self.warmup_epochs)
    ...
    optim = torch.optim.AdamW(
        ...
        lr=0.001,
        weight_decay=1e-4)
    warmup_scheduler = WarmupScheduler(optim, warmup_epochs, initial_lr, final_lr)
    ...
    while epoch<args.max_epoch:
        ...
        if epoch < warmup_epochs:
            warmup_scheduler.step() # Scheduler trying to create a warmup phase
        else:
            scheduler.step(val_loss) # Main scheduler post 10 epochs which reduces lr if val_loss plateau
        # scheduler.step(val_loss)

        print('mse loss:{}'.format(mse_valid))
        if epoch % 10 == 0:
            torch.save(model.state_dict(), 'IDDP_model/{}/{}.pth'.format(datetime,str(epoch)))
        if epoch+1==args.max_epoch:
            torch.save(model.state_dict(), 'IDDP_model/{}/IDDP.pth'.format(datetime))
        epoch+=1
...
if __name__=='__main__':
    ...
    print('models are saved in IDDP_model/{}'.format(datetime))
    main()
