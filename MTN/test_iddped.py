...
def test(model_path):
    ...
    test_dataset=baselineUtils.create_iddp_dataset(mean_tr,std_tr,mean_speed,std_speed,dataset='iddp',flag='test',**args.model_opts)
    ...
    def split_result_by_time_ranges(test_result):
        # averages = []
        
        for time_range in [15, 30, 45]:
            avg = test_result[:, :time_range, :].mean(axis=(0, 1, 2))
            # averages.append(avg)
            print(f'Average MSE for first {time_range} time steps: {avg}')
    ...
    with torch.no_grad():
        ...
        # split_resultbytime(mse)
        split_result_by_time_ranges(mse)
...
if __name__=='__main__':
    model_path='/scratch/ruthvik/MTN/IDDP/IDDP_model/2024-09-14-08-09-05/IDDP.pth'
    test(model_path)