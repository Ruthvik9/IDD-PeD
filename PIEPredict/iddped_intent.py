...
class IDDPIntent():
    """
    A convLSTM encoder decoder model for predicting pedestrian motive/intention
    Attributes:
    ...
    Methods:
        ...
        iddp_convlstm_encdec: generates intention prediction model
    ...
    def get_path(self,
                 ...
                 data_subset='',
                 data_type='',
                 save_root_folder=os.environ['IDDP_PATH'] + '/data/'):
        """
        ...
        if type_save == 'models':
            save_path = os.path.join(save_root_folder, 'iddp', 'intention', models_save_folder)
            if not os.path.exists(save_path):
                os.makedirs(save_path)
            return os.path.join(save_path, file_name), save_path
        else:
            save_path = os.path.join(root, 'iddp', data_subset, data_type, model_name)
            if not os.path.exists(save_path):
                os.makedirs(save_path)
            return save_path
    ...
    def load_images_and_process(self,
        ...
        # print("\n\n\n\ndimensions of output of context model\n\n\n\n",len(sequences),sequences.shape)
        return sequences
    ...
    def get_tracks(self, dataset, data_type, seq_length, overlap):
        ...
        # Note - There are many cases whether the length of bbox is 1. These are the cases where the track starts right
        # from the crossing point. (In the first frame itself, the pedestrian is crossing.)

        # print("KKKKKKKKKKK",d.keys()) In this run, the keys were bbox and intention_prob or intention_bin, I don't remember.
        for k in d.keys():
            tracks = []
            for track in d[k]:
            ...
            # The len(track) - seq_length + 1 in the range ensures that tracks shorter than seq_length are not even added to the overall final tracks,
            # thereby ensuring that the total length of each track equals the sequence_length.
            d[k] = tracks
    ...
    def get_train_val_data(self, data, data_type, seq_length, overlap):
        ...
        # currently, 'data' does have tracks that are shorter, but they don't contribute to the tracks. For why, check comments in get_tracks
    ...
    def get_test_data(self, data, train_params, seq_length, overlap):
        ...
        :return: A unified data representation as a list.
        """
        # bboxes_one = data['bbox']
        # for index,i in enumerate(bboxes_one):
        #     if len(i) < seq_length:
        #         print("--------------------------------------")
        #         print("--------------------------------------")
        #         print("Okie, not long enough. And the index is",index,"len of seq is",len(i))
        #         print("--------------------------------------")
        #         print("--------------------------------------")
        """
        Turns out, the output = output[:, 0]
IndexError: too many indices for array 
error arose because the shape of output was 0 (empty) cuz the shape of the inputs was 0 (empty),
because the test data is processed in chunks and the final chunk of 12 pedestrians, when it went through the get_tracks,
it returned nothing because co-incidentally the tracks of all these 12 pedestrians were shorter than 15 lol. Yes, bad coincidence, but it
made you go through the code properly atleast.
        """
        ...
        # Check to ensure whether the tracks are non-empty. They become empty when all the testing examples of the chunk
        # have an observation period of less than 15 frames.
        if len(images) == 0:
            return 'empty', 'empty'
        ...
        # print("type of tracks",type(tracks))
        # print("len of tracks",len(tracks))
        # print("params output type",train_params['data_type']['output_type'])
        ...
        # print("Contents of output:", output)
        output = output[:, 0]
        return ([test_img, decoder_input], output)

    def get_model(self, model):
        train_model = self.iddp_convlstm_encdec()
        return train_model
    ...
    def test_chunk(self,
            ...
            # print("looking at what happens",num_samples)
            vis_results = []

            for i in range(0, len(data_test['image']), 100): # chunks of 100 samples (those 100 may generate variable number of tracks ofc based on their lengths.)
                data_test_chunk = {}
                ...
                if test_data_chunk == 'empty': # If all tracks in the chunk don't have a sufficient obs. period, test_data_chunk is 'empty'
                    continue
                ...
                # To vis results for the first sequence for debugging
                # print(test_results_chunk[0])
                # print(len(test_results_chunk[0]))
                # try:
                #     print(test_results_chunk[0].shape)
                # except TypeError:
                #     pass
                ...
                # print("Reached here too! Seems to be an issue with a particular chunk")
