"""
Code modified for IDDP by Ruthvik.
...
class IDDPPredict(object):
    ...
    Methods:
        ...
        iddp_encdec: Generates the network model
    ...
    def get_tracks(self, dataset, data_types, observe_length, predict_length, overlap, normalize):
        ...
        JAAD has 'bbox', 'ceneter' and PIE and IDDP in addition has 'obd_speed', 'heading_angle', etc.
    ...
    def get_data(self, data, **model_opts):
        ...
        # I got an error because when training the speed model also this code runs and 'intention_prob' is not a key.
        # print("el suspico")
        # print(len(data_tracks['image']))
        # print(len(data_tracks['intention_prob']))
        # print("end of el suspico") 

        # print('lessgo',opts['enc_input_type'],opts['dec_input_type'])
        # Note- dec_input_type is ['intention_prob','obd_speed']. Hence the '2' in the dims. 
        ...
        # print('lmfao')
        # print(len(obs_slices['image']))
        # print(len(pred_slices['image']))
        # print(len(obs_slices['bbox']))
        # print(len(pred_slices['intention_prob']))
    ...
    def train(self, data_train, data_val,
        ...
        iddp_model.compile(loss=loss, optimizer=optimizer)
        ...
        history = iddp_model.fit(x=train_data[0], y=train_data[1],
    ...
    def generate_vis_results(self,index,results_org,test_data,test_target_data_org):
        print("Generating PIE traj outputs")
        # print(np.array2string(results_org[index],separator=','))
        np.save("PIE_traj.npy",results_org[index])
        # print(test_target_data_org.shape)
        # print("gt prediction") # Unnormalized gt prediction
        ob_15 = test_data['enc_input'][index]
        gt_45 = test_target_data_org[index]
        gth = np.concatenate((ob_15,gt_45),axis=0)
        # print("obs dims",test_data['enc_input'].shape)
        print("Generating ground truth boxes") # Unnormalized bbox observation sequence.
        np.save("gt_boxes.npy",gth)
        # print(np.array2string(gth,separator=','))
        # print("Images")
        # all_images = test_data['obs_image'][index]
        # all_images.extend(test_data['pred_image'][index])
        # print(all_images)
        # print(test_data['pred_image'][index])

    def qualitative_results(self,index,results_org,test_data,test_target_data_org):
        print("Generating PIE traj outputs")
        print("PIE Output")
        print(np.array2string(results_org[index],separator=','))
        # print(test_target_data_org.shape)
        ...
        # all_images = test_data['obs_image'][index]
        # all_images.extend(test_data['pred_image'][index])
        # print(test_data['obs_image'][index]) # 15 observation frames
        print(test_data['obs_image'][index][0])
        # print(test_data['pred_image'][index])

    def test_final(self, data_test, traj_model_path='', intent_model_path='', speed_model_path=''): # removed vid_num as first arg
        ...
        model_opts = {'normalize_bbox': True,
                       'track_overlap': 0.5,  # 0.8 for jaad, 0.5 pie and iddp
                       'observe_length': 15,
                       'predict_length': 45,
                       'enc_input_type': ['bbox'],
                       'dec_input_type': [],
        ...
        # for i,d in enumerate(box_data['obs_image']):
        #     if 'gp_set_0005' in d[0]:
        #         print(i,d)
        # print(box_data['obs_pid'][0])
        # print(box_data['pred_pid'][0])
        # print("Keys of box data",box_data.keys())
        # for k in box_data.keys():
        #     print("For key",k)
        #     if isinstance(box_data[k],np.ndarray):
        #         print("Dimensions are",box_data[k].shape)
        #         print("instance is",box_data[k][0])
        #     else:
        #         print("Dims are",len(box_data[k]))
        #         try:
        #             print('instance is',box_data[k][0])
        #         except:
        #             pass
        ...
        # print("Dims of test results",test_results[0].shape)
        ...
        #####################################################
        # Code to print the data to be used for qualitative results
        # Unnormalized bboxes for prediction
        # print("predicted vals ")
        # print(np.array2string(results_org[0],separator=','))
        # # print(test_target_data_org.shape)
        # print("gt prediction") # Unnormalized gt prediction
        # print(np.array2string(test_target_data_org[0],separator=','))
        # # print("obs dims",test_data['enc_input'].shape)
        # print("obs seq") # Unnormalized bbox observation sequence.
        # print(np.array2string(test_data['enc_input'][0],separator=','))
        # print("obs sequences")
        # print(test_data['obs_image'][0])
        # print(test_data['pred_image'][0])
        #######################################################
        self.qualitative_results(20,results_org,test_data,test_target_data_org)
        # self.generate_vis_results(vid_num,results_org,test_data,test_target_data_org)
        #  Performance measures for centers
        res_centers = np.zeros(shape=(test_results.shape[0], test_results.shape[1], 2))
        centers = np.zeros(shape=(test_results.shape[0], test_results.shape[1], 2))
        for b in range(test_results.shape[0]):
        ...
        # Commenting out for plotting start
        ...
        #     with open(save_results_path, 'wb') as fid:
        #         pickle.dump(results, fid, pickle.HIGHEST_PROTOCOL)
        # Commenting out for plotting end
        return perf  # performance

    def iddp_encdec(self):
        """
