...
# environ['OPENCV_FFMPEG_READ_ATTEMPTS'] = '10000'
...
class IDDPedestrian():
    ...
    def process_xml_files(self,directory):
        ...
        if exists(join(directory,f'{set_name}_annotated_frames.csv')):
            raise Exception(f'{set_name}_annotated_frames.csv already exists. Delete the file and try again.')
        
        with open(join(directory,f'{set_name}_annotated_frames.csv'), 'w', newline='') as csvfile:
    ...
    def get_annotated_frame_numbers(self, set_id):
        ...
        if isfile(annotated_frames_file):
            ...
            raise Exception(f'{annotated_frames_file} does not exist. Generate the file first if  it doesn\'t already exist using create_annotated_frame_numbers()')
    ...
    def extract_and_save_images(self, extract_frame_type='annotated',lib='opencv'):
        ...
        for cam in cams:
            ...
            print("############################################")
            set_folders = listdir(join(self._videos_path,cam)) # To only get the sets which have annotations.
            for set_id in set_folders:
    ...
    def _map_scalar_to_text(self, label_type, value):
        ...
        map_dic = {'occlusion': {0: 'None', 1: 'Part', 2: 'Full'},
                   ...
                   'crossing_motive': {0: 'yes', 1: 'maybe', 2: 'no'},
    ...
    def _get_annotations(self, setid, vid):
        ...
        for t in tracks:
            ...
            if obj_id == None:
                # print(f'object id none lololP{obj_id}',setid,vid)
                # print("Skipping")
                continue
            ...
            for vid in video_ids:
                ...
                for ped in database[setid][vid]['pedestrian_annotations']:
                    try:
                        ...
                        print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
                        print(f"Key {ped} not found, kindly check")
                        print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
                        continue
    ...
    def get_data_stats(self,plot=False):
        ...
        social_dynamics = {'GS': 0, 'CFA': 0, 'N/A': 0}
        ...
        if plot == True:
            ...
            self.plot_basic_counts(counts, labels, 'basic_counts')
            ...
            for title,var in all_dicts.items():
                self.plot_dict_with_colors(var, f'{title} Distribution', f'{title}', f'{title}_distribution')
            ...
            for title,var in time_series_dicts.items():
                self.plot_time_series(var, f'{title} Distribution', f'{title}', f'{title}_distribution')

            



    def plot_basic_counts(self,counts, labels, file_name):
    ...
    def plot_dict_with_colors(self,data, title, xlabel, file_name):
        keys = list(data.keys())
    ...
    def plot_nested_dict(self,data, title,file_name):
    ...
    def plot_time_series(self,data, title, xlabel,file_name):
            ...
            for vid in annotations[sid].keys():
                for pid in annotations[sid][vid]['pedestrian_annotations'].keys():
                    ...
                    try:
                        ...
                        continue
                    if self._map_scalar_to_text('signalized_type', value) in ['C','S','CS']:
                        output = 'CS'
                    if category == output:
                        pids.append(pid)
        return pids
    ...
    def _beh_filter(self,height_rng,frame_ids,boxes,images,occlusions,beh_labels,occluded=False):
        ...
        for i, b in enumerate(boxes):
            bbox_height = abs(b[1] - b[3])
            if height_rng[0] <= bbox_height <= height_rng[1] and occlusions[i] <= 1:
                box.append(b)
                imgs.append(images[i])
                frames.append(frame_ids[i])
    ...
    def generate_data_trajectory_sequence(self, image_set, **opts):
        ...
        params = {'fstride': 1,
                  ...
                  'min_track_size': 16, # 16 frame observation period as outlined in the evaluation protocol.
        ...
        for sid in set_ids:
            for vid in sorted(annotations[sid]):
                ...
                for pid in sorted(pid_annots):
                    ...
                    occ_start = len(occlusions) - 76
                    occ_end = len(occlusions) - 30
                    occlusions_imp = occlusions[occ_start:occ_end]
