...
import cv2
from utils import *
from base_models import AlexNet, C3DNet, I3DNet, convert_to_fcn, C3DNet2
...
def speed_average_map(seq):
    """
    Takes a sequence of speeds and maps it to a number based on the average.
    seq is a list of speeds (that correspond to one sample/sequence.)
    """
    mean_speed = np.mean(seq)
    if int(mean_speed) == 0:
        return 0
    elif int(mean_speed) <= 5:
        return 1
    elif int(mean_speed) <= 10:
        return 2
    else:
        return 3
    # elif int(mean_speed) <= 30:
    #     return 3
    # elif int(mean_speed) <= 40:
    #     return 4
    # elif int(mean_speed) <= 50:
    #     return 5
    # elif int(mean_speed) <= 60:
    #     return 6
    # else:
    #     return 7

def group_map(group_size):
    if group_size == 1:
        return 1
    elif group_size == 2:
        return 2
...
class ActionPredict(object):
    ...
    def get_data_sequence(self, data_type, data_raw, opts):
        ...
        d = {'center': data_raw['center'].copy(),
             ...
             'crossing': data_raw['activities'].copy(), # The ground truth labels.
             'image': data_raw['image'].copy()}

        # balance = opts['balance_data'] if data_type == 'train' else False
        balance = opts['balance_data']
        ...
        if balance:
            self.balance_data_samples(d, data_raw['image_dimension'][0][0]) # data_raw['image_dimension'][0] is essentially the width of the image.
            # The above balancing happens, in-place, for the keys of dictionary d.
        d['box_org'] = d['box'].copy() # We use this later in get_context_data().
        non_overlap_box_seqs = d['box'].copy()
        d['tte'] = []
        ...
        try: # Either get it from data raw if it exists else initiaze to empty list
            d[category_name] = data_raw[category_name].copy()
        except:
            d[category_name] = [] # Similar to tte, a list to store the metadata associated with each testing sample, so that later we can group by this.
        ...
        if isinstance(time_to_event, int):
            ...
            d['tte'] = [[time_to_event]]*len(d['box'])
            # Logic for category thing still to be implemented
            if opts['category_name'] in d.keys():
                raise NotImplementedError
        else:
            overlap = opts['overlap'] # if data_type == 'train' else 0.0
            olap_res = obs_length if overlap == 0 else int((1 - overlap) * obs_length)
            olap_res = 1 if olap_res < 1 else olap_res # minimum overlap stride of 1.
            if category_name != 'post_cross': # Sequences pre-crossing are to be generated.
                for k in d.keys():
                    seqs = []
                    for seq in d[k]:
                ...
                for seq in non_overlap_box_seqs:
                    start_idx = len(seq) - obs_length - time_to_event[1]
                    end_idx = len(seq) - obs_length - time_to_event[0]
                    d['tte'].extend([[len(seq) - (i + obs_length)] for i in
                                    range(start_idx, end_idx + 1, olap_res)]) 

            else: # category_name is post_cross, so data post crossing is to be generated.
                overlap = 0.8 # will result in olap_res of 3 frames.
                olap_res = obs_length if overlap == 0 else int((1 - overlap) * obs_length) 
                for k in d.keys():
                    seqs = []
                    for seq in d[k]:
                        start_idx = 0 # Since the seqs will be post crossing begin, 0 corresponds to immediately after cross-start.
                        end_idx = 60 # We can make these - params - but for now I'm hard coding them.
                        seqs.extend([seq[i:i+obs_length] for i in
                                     range(start_idx, end_idx+1,olap_res)])
                    d[k] = seqs
                    
                for seq in non_overlap_box_seqs:
                    # For every sequence, we are adding the following values to the dict.
                    d[category_name].extend([[i] for i in range(start_idx,end_idx+1,olap_res)]) # Category name would be post cross.

                

            if category_name == 'vehicle_speed':
                for i in d['speed'].copy():
                    d[category_name].extend([[speed_average_map(i)]])
        ...
        # So d['tte'] is like [[60],[54],[48],[42],[36],[30],[60],........] with each element corresponding to the tte 
        # for the corresponding sequence.
        print('Len of sequence before normalization',len(d['image'][0]))
        if normalize:
            for k in d.keys():
                if k != 'tte' and k != category_name: # Cuz there's no normalizing needed wrt tte values or category values.
                    if k != 'box' and k != 'center':
                        for i in range(len(d[k])):
                            d[k][i] = d[k][i][1:]
                    else:
                        for i in range(len(d[k])):
                            d[k][i] = np.subtract(d[k][i][1:], d[k][i][0]).tolist() # To capture the relative change wrt starting frame
                d[k] = np.array(d[k])
        else:
            for k in d.keys():
                d[k] = np.array(d[k])
        print('Len of sequence after normalization',len(d['image'][0]))

        d['crossing'] = np.array(d['crossing'])[:, 0, :] # The ground truth labels.
    ...
    #     if normalize:
    #         for k in d.keys():
    #             if k != 'tte': # Cuz there's no normalizing needed wrt tte values.
    #                 if k != 'box' and k != 'center':
    #                     for i in range(len(d[k])):
    #                         d[k][i] = d[k][i][1:]
    #                 else:
    ...
    def balance_data_samples(self, d, img_width, balance_tag='crossing'):
        ...
        gt_labels = [gt[0] for gt in d[balance_tag]] # d[balance_tag] (d['crossing'] here is basically the ground truth array.)
        print("Num of sequences are",len(gt_labels))
        ...
        if num_neg_samples == num_pos_samples:
            ...
            if num_neg_samples > num_pos_samples:
                gt_augment = 1 # Label to be augmented.
            else:
                gt_augment = 0

            num_samples = len(d[balance_tag])
            # As of now, d has 6 keys - center, box, image, speed, ped_id, crossing.
            for i in range(num_samples):
                if d[balance_tag][i][0][0] == gt_augment:
                    for k in d:
                        if k == 'center':
                            flipped = d[k][i].copy()
                            flipped = [[img_width - c[0], c[1]] # cuz when flipping wrt a vertial axis, the x coordinate changes but not the y coordinate.
                                       for c in flipped]
                            d[k].append(flipped)
                        if k == 'box':
                            flipped = d[k][i].copy()
                        ...
                        if k == 'image':
                            ...
                            d[k].append(flipped)
                        if k in ['speed', 'ped_id', 'crossing']: 
                            # d doesn't have the keys 'walking', 'looking' even in the PIE implementation. It's an oversight from them.
                            # But if you want to get the behavioral attributes as well, then yes, you have to include yours here.
            ...
            print("Finished augmenting samples to balance positive and negative examples")
    ...
    def get_context_data(self, model_opts, data, data_type, feature_type):
        ...
        if 'flow' in feature_type:
            ...
            print(len(data['image']),len(data['box_org']),len(data['ped_id']))
            return self.load_images_crop_and_process(data['image'],
                                                     data['box_org'],
                                                     data['ped_id'],
                                                     process=process,
                                                     **data_gen_params)
    def get_data_categories(self, data_type, data_raw, model_opts,category_name='tte'):
        """
        ...
        Args:
            ...
            category_name: Name of category to group results by
        Returns:
        ...
        process = model_opts.get('process', True)
        dataset = model_opts['dataset']
        model_opts['category_name'] = category_name
        ...
        for d_type in model_opts['obs_input_type']: # The things we give in the config files. We iterate over each d_type and get it.
            if 'local' in d_type or 'context' in d_type or 'mask' in d_type:
                ...
                features = get_pose(data['image'],
                                    data['ped_id'],
                                    data_type=data_type, # data_type is basically 'train', 'test' or 'val'.
                                    file_path=path_to_pose,
                                    dataset=model_opts['dataset'])
                feat_shape = features.shape[1:] # I think this will be 16 X 34 (features.shape[1:] i.e.)
            else: # if the modalities are already present in the 'data' dictionary - like bboxes, speed etc.
    ...
    # Test Functions
    def test_categories(self, data_test,category_name='tte', model_path=''):
        """
        Evaluates a given model
        Args:
            data_test: Test data
            category_name: Name of the category according to which we group results
        ...
        print("Category being considered",category_name)
        test_data = self.get_data_categories('test', data_test, {**opts['model_opts'], 'batch_size': 1},category_name=category_name)


        # Extract category values and unique categories
        category_values = np.array(test_data[category_name])
        # print(category_values)
        unique_category_values = np.unique(category_values)

        test_results = test_model.predict(test_data['data'][0],
                                          batch_size=1, verbose=1)
        
        # Replace nan values with 0 and print the number of nan values.
        nan_mask = np.isnan(test_results)
        print("Number of nan values",nan_mask.sum())
        test_results[nan_mask] = 0.0

        for category in unique_category_values:
            # Filter indices of test points with the current category
            category_indices = np.where(category_values == category)[0]
            print(f"Number of samples of category {category_name} - {category} are",len(category_indices))

            # Extract labels and results for this category
            true_labels = test_data['data'][1][category_indices]
            predicted_labels = np.round(np.array(test_results)[category_indices])

            # Calculate metrics
            accuracy = accuracy_score(true_labels, predicted_labels)
            f1 = f1_score(true_labels, predicted_labels)
            if category_name != 'post_cross': # cuz if it is, there will only be samples from one class (cross)
    ...
    def find_misclassifications(self,test_data, test_results):
        ground_truth = test_data['data'][1]
        predictions = np.round(test_results)
        
        # Find indices where predictions don't match ground truth
        misclassified_indices = np.where(ground_truth != predictions)[0]
        
        # Create list of tuples with (image, ped_id, ground_truth_label, predicted_label) for misclassified instances
        misclassifications = [
            (test_data['image'][i][0], test_data['ped_id'][i][0], ground_truth[i], predictions[i])
    ...
    def test(self, data_test, model_path=''):
        ...
        acc = accuracy_score(test_data['data'][1], np.round(test_results))
            ...
            # Use the function
        wrong_classifications = self.find_misclassifications(test_data, test_results)

# Print results
        # print(f"Number of misclassifications: {len(wrong_classifications)}")
        print("List of (image, ped_id, ground_truth, prediction) for misclassified instances:")
        for image, ped_id, true_label, pred_label in wrong_classifications:
            print("Image: {}, Pedestrian ID: {}, Ground Truth: {}, Predicted: {}".format(image,ped_id,true_label,pred_label))
...
class C3D(ActionPredict):
    ...
    def get_data_categories(self,data_type,data_raw,model_opts,category_name='tte'):
        ...
        model_opts['category_name'] = category_name
        return super(C3D, self).get_data_categories(data_type, data_raw, model_opts)
    ...
    def get_model(self, data_params):
        if not os.path.exists('./weights'):
            os.makedirs('./weights')
            print(f"Directory 'weights' was created.")
        else:
            print(f"Directory 'weights' already exists. Downloading weights there.")
