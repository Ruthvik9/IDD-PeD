...
def flip_pose(pose):
    ...
    #						 Lear(32,33), Rear(34,35)] - in the original implementation
    ...
    flip_map = [0, 1, 4, 5, 2, 3, 8, 9, 6, 7, 12, 13, 10, 11, 16, 17, 14, 15, 20, 21,
                18, 19, 24, 25, 22, 23, 28, 29, 26, 27, 32, 33, 30, 31]
    new_pose = pose.copy()
...
def get_pose(img_sequences,
    ...
    for s in set_poses_list:
        ...
        if dataset == 'pie' or dataset == 'jaad':
            set_poses[s.split('.pkl')[0].split('_')[-1]] = p
        elif dataset == 'iddp':
            set_poses[s.split('.pkl')[0][5:]] = p
    i = -1
    for seq, pid in zip(img_sequences, ped_ids):
        ...
        for imp, p in zip(seq, pid):
            ...
            if dataset == 'pie':
                ...
                set_id = imp.split('/')[-3] # cuz data/iddp/images/gopro/set/vid/image_name, so -3 is set
            ...
            # Deal with the case where when tracks are considered till the end, there are vids for which poses are not available.
            if vid_id not in set_poses[set_id].keys():
                pose.append([0] * 34)
                continue
