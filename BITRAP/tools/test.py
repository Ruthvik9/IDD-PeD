# Usage - python tools/test.py --config_file configs/bitrap_np_IDDP.yml CKPT_DIR /scratch/ruthvik/BITRAP/checkpoints/IDDP_checkpoints/goal_cvae_checkpoints/no_wandb/Epoch_049.pth
...
def main(cfg): # Removed video_num
    # build model, optimizer and scheduler
    model = make_model(cfg)
    model = model.to(cfg.DEVICE)
    if os.path.isfile(cfg.CKPT_DIR):
        model.load_state_dict(torch.load(cfg.CKPT_DIR))
        print('Loaded checkpoint:{}'.format(cfg.CKPT_DIR))
    else:
        print('The cfg.CKPT_DIR id not a file: {}'.format(cfg.CKPT_DIR))
    ...
    inference(cfg, 0, model, test_dataloader, cfg.DEVICE, logger=logger, eval_kde_nll=True, test_mode=True) # Removed vid_num


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="PyTorch Object Detection Training")
    # parser.add_argument('--video_num',type=int)
