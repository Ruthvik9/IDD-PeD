...
def evaluate_bbox_traj(pred_traj, target_traj, bbox_type='x1y1x2y2'):
    ...
    if pred_traj.shape != target_traj.shape or len(pred_traj.shape) != 4:
        raise ValueError("input shapes are not correct!")
    eval_results = {}
    ...
    # print("Dims of per_step_box_mse",per_step_box_mse.shape) # (B,15,20,4)
    # NOTE: mean over dim and time, then min over K samples, then mean over batch

    # eval_results['ADE(0.5s)'] = per_step_box_mse[:, 0:5].mean(axis=(1, 3)).min(axis=1).mean()
    # eval_results['ADE(1.0s)'] = per_step_box_mse[:, 0:10].mean(axis=(1, 3)).min(axis=1).mean()
    # Commented out the below cuz they have a different evaluation protocol.
    ...
    # Additional code added by Ruthvik to get the best and worst indices for 
    # each sample in the batch.
    mean_mse = per_step_box_mse.mean(axis=(1, 3)) # (B,K)
    # Find the indices of the best and worst predictions for each sample in the batch
    best_indices = mean_mse.argmin(axis=1)  # shape: (B,)
    worst_indices = mean_mse.argmax(axis=1)  # shape: (B,)

    mse_45 = mean_mse.min(axis=1) # Shape: (B,)
    eval_results['ADE(1.5s)'] = mse_45.mean()
    mse_last = per_step_box_mse[:, -1].mean(axis=-1).min(axis=1)
    eval_results['FDE'] = mse_last.mean()
    eval_results['best_indices'] = best_indices
    eval_results['worst_indices'] = worst_indices
    ...
    # eval_results['C-ADE(0.5s)'] = per_step_center_mse[:, 0:5].mean(axis=(1, 3)).min(axis=1).mean()
    # eval_results['C-ADE(1.0s)'] = per_step_center_mse[:, 0:10].mean(axis=(1, 3)).min(axis=1).mean()
