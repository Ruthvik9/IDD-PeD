...
def viz_results(viz, 
    ...
    if y_global.shape[-1] == 2:
        ...
        # print('eother two') # entered this
        viz_img = viz.img

    if hasattr(logger, 'log_image'):
        # print("Entered the logggg phase") # didnt enter this
        logger.log_image(viz_img, label=name)
    else:
        # Convert viz_img to a PIL Image if it's not already
        if isinstance(viz_img, np.ndarray):
            viz_img = Image.fromarray(viz_img)
        
        # Save the image
        image_path = f"{id_to_show}.png"  # or any other desired file path
        viz_img.save(image_path)
...
def post_process(cfg, X_global, y_global, pred_traj, pred_goal=None, dist_traj=None, dist_goal=None):
    ...
    if hasattr(dist_goal, 'mus'):
        ...
        dist_goal.squeeze(1)
    if dim == 4:
        # print("Entered the denorm. we are back to square one lmao") entered this
