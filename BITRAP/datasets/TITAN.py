...
from datasets.titan_interface_ped import TITAN
from datasets.iddp_interface_traj import IDDPedestrian
...
class TITANDataset(data.Dataset):
    def __init__(self, cfg, split):
        ...
        traj_model_opts = {'normalize_bbox': True,
                       'track_overlap': 0.5,
                       'observe_length': 5, # Corresponding to 0.5 sec
                       'predict_length': 15, # Corresponding to 1.5 sec
                       'enc_input_type': ['bbox'],
                       'dec_input_type': [], 
        ...
        traj_model_opts['enc_input_type'].extend(['obd_speed']) # Removed heading angle since our data doesn't have it
        traj_model_opts['prediction_type'].extend(['obd_speed'])
        beh_seq = imdb.generate_data_trajectory_sequence(self.split, **traj_data_opts) # generates raw sequences
    ...
    def get_traj_tracks(self, dataset, data_types, observe_length, predict_length, overlap, normalize):
        ...
        # Commenting out the below as per - https://github.com/umautobots/bidireaction-trajectory-prediction/issues/13
        ...
        for k in d.keys():
            ...
            d[k] = tracks
        # print("Orign lets see",d['bbox'][10]) same as the ones we got post denormalization and reverting format conversion.
        #  Normalize tracks using FOL paper method, 
        d['bbox'] = self.convert_normalize_bboxes(d['bbox'], 
    ...
    def convert_normalize_bboxes(self, all_bboxes, normalize, bbox_type):
        ...
        for i in range(len(all_bboxes)):
            ...
            # NOTE ltrb to cxcywh
            if bbox_type == 'cxcywh':
                # print("huehue1") entering this
                # the below code converts x1y1x2y2 into the cxcywh format.
                bbox[..., [2, 3]] = bbox[..., [2, 3]] - bbox[..., [0, 1]]
                bbox[..., [0, 1]] += bbox[..., [2, 3]]/2
            # NOTE Normalize bbox
            if normalize == 'zero-one':
                # print("huehue2") entering this
    ...
    def get_traj_data(self, data, **model_opts):
        ...
        for k in data_tracks.keys():
            ...
            print("Downsample is",down) # down is 1
